...
3. Open the [src/index.ts](/blob/main/src/index.ts) file and import the above created functions.
4. Inside the same [src/index.ts](/blob/main/src/index.ts) file and below the init() function, you will see 4 private functions `addLicence`, `addSubscription`, `addPayment` and `addCustomer`. These functions handle the code writing to the database. Now, inside each of these functions, to add your database, do:
```ts
...
Still stuck?
Check out the [MongoDB integration](/blob/main/src/database-integrations/mongodb/index.ts) file for example.
